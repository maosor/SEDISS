...
<?php if (!isset($_GET['c']) && (!isset($_GET['f'])||$_GET['f']=="undefined")) {
  ...
  echo "Cuadro N° 3B: Productividad y Composición del Recurso Humano -Servicios de Apoyo <br>";
  ...
  <div class="row"> <!-- Costos directos-->
      ...
      $index = 0;
      $total=0;
      $sel_unid = $con->prepare("SELECT p.producto, uo.Descripcion FROM produccion p INNER JOIN tipo_unidades_gestion uo ON p.producto = uo.id
          WHERE p.unidadgestion <> p.producto AND p.idcompania = ? AND p.organizacion = ? AND p.fecha = ?
          GROUP BY p.producto ORDER BY funcion , CONCAT(0,(SELECT orden FROM tipo_unidades_gestion WHERE id = uo.idpa), uo.orden) * 1");
          ...
          <div class="divTableBody">
            ...
            <div class="divTableCell number" style="width: 140px;"><b>TOTAL</b></div>
             ...
             $primerainteraccion=true;
              while ($sel_unid ->fetch()):
                $sel_insum = $con->prepare("SELECT uo.Descripcion as PRODUCCION_HRS_RRHH, sum(p.rubro)  as Volumen_Produccion,p.primaria
                   FROM produccion p INNER JOIN tipo_unidades_gestion uo on p.producto = uo.id
                    WHERE p.unidadgestion <> p.producto AND p.primaria = 1 AND p.idcompania = ? AND p.organizacion = ? AND p.fecha = ? and p.producto = ?
                    GROUP BY p.producto, p.primaria UNION
                    SELECT 'RECURSO NUCLEAR', sum(h.rubro),'2' FROM horas h INNER JOIN red_organizacional i
                   ON h.recurso = i.id AND h.idcompania =? AND h.organizacion =? AND fecha = ? AND h.unidadgestion = ?
                   WHERE EXISTS(SELECT 1=1 FROM horas WHERE rubro > 0 AND fecha = ? AND recurso = h.recurso) GROUP BY h.unidadgestion UNION
                 ...
                 $sel_insum -> bind_param('iisiiisisiisis', $compania,$val_org,$fecha,$idug, $compania,$val_org,$fecha,$idug, $fecha, $compania,$val_org,$fecha,$idug,$fecha);
                  $sel_insum -> execute();
